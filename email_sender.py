...
def login():
    if validate_login():
        ...
        global server
        server = smtplib.SMTP('smtp.gmail.com:587') #insecure SMTP connection
        server.ehlo() #identifies user to SMTP server
...
def send_mail():
    if validate_message():
        ...
        try:
            ...
            label9.grid()
            label9['text'] = 'Error in sending email'
            root.after(10, label9.grid)
...
def validate_message():
    ...
    if (email_text == "") or (sub_text == "") or (msg_text == ""):
        ...
        if not EMAIL_REGEX.match(email_text):
            label9.grid()
            label9['text'] = "Enter a valid email"
            root.after(10, root.grid)
            return False
        elif(len(sub_text) < 3) or (len(msg_text) < 3):
            label9.grid()
            label9['text'] = "Enter at least three characters"
            root.after(10, root.grid)
            return False
        else:
            return True
...
label4 = Label(f2, width=20, bg='cyan', fg='red', text='Logged in!', font=("Calibri 12 bold"))
